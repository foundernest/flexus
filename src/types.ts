...
export type ArgumentTypes<F extends Action<any>> = F extends (
  ...args: infer A
) => any
  ? A
  : never
...
> = ActionCreatorOrMap extends ActionCreator<string>
  ...
  ? ActionMap<ActionCreatorOrMap>[keyof ActionCreatorOrMap]
  : never
...
export declare type ActionMap<T> = { [K in keyof T]: Action<T[K]> }
...
> = {
  Store: React.Context<
    LocalStoreParams<State, AvailableActionType, EffectsHandler>
  >
  Consumer: FunctionComponent<{}>
  Provider: React.ComponentType<{}>
}

export type Effect<State extends Object, AvailableActions> = (
  ...args: any
) => EffectDispatch<State, AvailableActions>
export type EffectDispatch<State extends Object, AvailableActions> = (
  params: EffectDispatchParams<State, AvailableActions>
) => void
